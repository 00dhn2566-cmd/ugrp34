...
### K_thrust/K_drag 추정 → PID 자동 스케일 (사용자 설계, 2026-07-15)

**개념**: 프로펠러 계수는 튜닝 노브가 아니라 플랜트 상수 — 대신 **입력 대비 출력으로 실측 추정**하고, PID는 그 계수에 대한 식으로 묶어 자동 보상한다.

- **수용구 (구현 완료, parameters.m)**: 게인 정규화 앵커 `Kthrust_ref=9.79`/`Kdrag_ref=0.597` (튜닝 당시 값, **갱신 금지**) + 스케일 `sT=Kthrust_ref/propeller.Kthrust`, `sQ=Kdrag_ref/propeller.Kdrag`. 자세·고도 게인 ×sT, yaw ×sQ. 현재 스케일=1이라 수치 무변화 (등가성 검증 완료). 이후 `propeller.Kthrust/Kdrag`에 새 추정치를 꽂으면 게인이 자동 재스케일되어 튜닝 형상(교차주파수) 보존.
- **추정기 (미구현 — 다음 단계)**: 입력↔출력 데이터로 계수 식별:
  - K̂_thrust: 호버 평형 — 측정 호버 모터속도 w_h에서 `m·g = 4·Kthrust·(공식항)·w_h²` 역산. 또는 동적: 명령 w² 변화 vs 측정 z̈ 회귀.
  - K̂_drag: yaw 토크 균형 — 차동 w² 입력 vs 측정 yaw 각가속 회귀 (관성 Izz 기지 전제), 또는 정상 회전에서 모터 토크 평형.
  - 산출물은 attitude_feedback.json에 `"k_est": {"kthrust": ..., "kdrag": ..., "confidence": ...}` 필드로 동승 → 신뢰도 임계 넘을 때만 parameters 반영 (급변 방지 저역 필터/램프 권장).
- 효용: 프롭 교체·배터리 새그·공기밀도 변화·실기 이전 시 재튜닝 없이 계수 재추정만으로 대응 — 강건 제어 과제 본론의 적응 요소.

### 실시간 상태 보고 파일 (사용자 지정, 2026-07-15 — 3번째 인터페이스)

**비행 내내 상시 갱신되는 현재 상태 보고 창구.** 특정 이벤트용이 아니라, 누구든(path_time 재계획, 상위 계층 모니터링/판단, 디버깅) 아무 때나 읽으면 드론의 지금 상태가 들어있는 단일 파일:

- **쓰기 (컨트롤러 측)**: `control_seoungjin/output/current_state.json` — **상시 덮어쓰기 갱신** (예: 20~50Hz. 시뮬 배치에선 To Workspace 로그를 주기 후처리로 흉내). **원자적 쓰기 필수**: 임시파일에 쓰고 rename — 독자가 반쯤 써진 JSON을 읽는 사고 방지.
- **읽기 (소비자들)**: ① path_time — 재계획 시 이 상태를 초기조건으로 **위치·속도·가속 연속 이어붙임** (안 하면 새 궤적 첫 샘플이 점프 = 미분 킥(§W ①) 자초) ② 상위 계층 — 임무 진행/충돌 판단용 현재 상태 조회 ③ 모니터링/디버깅
- **내용(안)**:
```json
{
  "timestamp": "2026-07-15T23-40-00.123",
  "pos": [x, y, z], "vel": [vx, vy, vz], "acc": [ax, ay, az],
  "yaw_rad": 0.0,
  "ref_state": { "pos": [...], "vel": [...], "acc": [...] }
}
```
- **⚠ 초기조건 선택 원칙 (§V 함정 재발 방지)**: 평시 재계획은 **`ref_state`(현재 성형 기준의 상태)**에서 이어붙일 것 — 측정 상태로 이어붙이면 궤적 생성에 측정 피드백이 섞여 성형기 원칙 1(피드백 성형 금지) 위반 소지. 측정 상태(`pos`/`vel`) 사용은 **드론이 기준에서 크게 이탈한 비상 재계획**(충돌 회피 후)에만, 스플라이스 구간을 온건(Tm≥0.9s)하게.
- **신선도 검사**: path_time은 timestamp 나이가 임계(예: 0.5s) 초과면 시끄럽게 error() (낡은 상태로 이어붙이기 금지).
