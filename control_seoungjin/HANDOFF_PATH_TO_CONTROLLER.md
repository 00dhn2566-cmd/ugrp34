...
3. 구현 분담(사용자 확정): **게이트는 xy 노름 검사**(엄격, `traj_gate.m`) / **성형기는 각 축 독립 적용**(`traj_smoother.m`). 성형기의 대각 √2배 낙관은 여유율(2.0 < 실측 2.5)과 게이트 노름 검사가 흡수.
...
- **발산 미해결 단서(§U)**: 클램프 우회하는 둘째 경로(기준→자세명령 직행, 피드포워드 의심) — **다음 세션 1순위(사용자 지시): 피드포워드 해부** (YPR 진입 직전 자세명령 태핑 + spline 소비처 전수 추적).
