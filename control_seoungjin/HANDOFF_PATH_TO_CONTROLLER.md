...
## 명령 스무더 설계 스펙 (사용자 확정, 2026-07-14 — 다음 세션 구현 항목)
...
3. x/y는 축별 말고 **‖a_xy‖ 벡터 노름**에 적용 (축별 2.5씩 주면 대각 이동 시 실효 3.5). z는 추력 예산 별도.
...
- **발산 미해결 단서(§U)**: 클램프 우회하는 둘째 경로(기준→자세명령 직행, 피드포워드 의심) 배선 추적이 다음 세션 1순위.
