...
## 5b. 실시간 능력 표 (`capability.json`) — schema_version 0.2
...
1. ~~**양쪽 끝이 아직 안 붙었다**~~ → **생산자는 붙었다** (`spec_governor.SpecGovernor`,
   §5c). C++ 쪽도 `QcOutput.spec` 으로 같은 수를 낸다. **소비자(계획기)는 여전히 안 읽는다** —
   `path_time`/`traj_pipeline` 이 `capability.json` 을 입력으로 받는 배선이 남았다.
2. **`observed.rho` 를 실제로 채우는 경로 없음** — 위 규정대로 제어기에서 뽑아야 한다.
   C++ 은 `QcOutput.rho` 로 내보내지만, 그걸 파이썬 조속기에 넣는 배관이 없다.
...
5. **지연 표본 주입 지점 미정** — `latency_tracker` 에 무엇을 넣을지(상태 파일 나이 /
   ...
   C++ 쪽은 `QcInput.measAgeS` 로 자리를 만들어 뒀지만 **채우는 쪽이 없다.**
6. **자세 경로 지연이 구성값이다** (`QcConfig.latencyAttS`, 기본 3 ms) — 온라인으로
   재지 않는다. 하드웨어 상수에 가깝다고 보고 그렇게 뒀는데, 실기에서 IMU 드라이버
   지터가 크면 이 가정이 깨진다. 게이트가 걸린 축이라 틀리면 조용히 위험해진다.
7. **0 kg 위치 지연표 미측정** — 1 kg 만 쟀다. 0 kg 은 질량이 작아 지연에 더 민감할
   수도, 관성이 작아 회복이 빠를 수도 있다. 추정하지 말고 재야 한다.
...
> **2026-08-23 정정 — 보고값은 `max(느린, 빠른)` 이다.** 느린 EMA 만 내보내면 지연이
> 붙은 직후 약 30 표본 늦게 반영된다. 그런데 아래 §5c 가 보이듯 감쇄를 결정한 **뒤에도**
> 새 한계 안으로 들어가는 데 0.7~4 s 가 더 걸린다 — 늦은 감지 + 늦은 수렴은 두 번 늦는
> 것이다. `max` 를 쓰면 붙는 순간은 빠른 EMA 가, 빠질 때는 느린 EMA 가 이겨서
> "즉시 깎고 천천히 되돌린다" 는 비대칭이 그대로 나온다.

---

## 5c. 지연 → 스펙 규칙 (2026-08-23 MATLAB 실측)

사용자 요구: *"시간 지연이 있어도 disturbance 에 대한 강건성은 계속 유지해야 한다.
예상 지연을 계속 업데이트하면서 상위 계획기에 보고할 spec 을 조정한다."*
**게인은 손대지 않는다** — 지연에 맞춰 스펙을 여유 있게 깎고 `path_time` 이 그 안에서
궤적을 만든다.

### 두 경로를 다르게 다룬다 (이 절의 요점)

| 경로 | 성격 | 근거 |
|---|---|---|
| **자세** (IMU→제어기) | **게이트** — 넘으면 임무 거부 | 20 ms 부터 *기동을 안 해도* 호버가 떨린다. 정지 상태의 불안정은 궤적을 느리게 해서 못 고친다 |
| **위치** (VIO→제어기) | **감쇄** — 배율로 깎는다 | 오차가 속도에 비례하므로 느려지면 준다 |

이게 실무적으로 중요한 이유: "스펙을 낮춰 버틴다"가 **안 먹히는 축이 하나 있다.**
그 축에서는 값을 깎는 대신 `degraded.mission_allowed = false` 로 상위에 알려야 한다.

### 자세 경로 실측 (`diagnose/sweep_delay_margin.m`, 1 kg, 제자리 호버 + 0.3 N·m x 0.3 s)

| 지연 [ms] | 0 | 8 | 12 | 16 | 20 | 24 |
|---|---|---|---|---|---|---|
| 호버 RMS [°] | 0.021 | 0.004 | 0.004 | **0.211** | **2.437** | 4.614 |
| 외란 피크 [°] | 2.32 | 2.50 | 2.60 | 3.08 | 4.59 | 7.10 |
| 판정 | OK | OK | OK | 아슬 | FAIL | FAIL |

→ `LAT_ATT_CLEAN_S = 0.012` (무보정) / `LAT_ATT_MAX_S = 0.016` (초과 시 거부) /
그 사이 `LAT_ATT_MARGIN_SCALE = 0.60`.

### 위치 경로 실측 (`diagnose/sweep_delay_spec.m`, 3 m 이동 중간에 같은 펄스)

각 지연에서 배율 `s` 를 1.0 부터 내리며 **종단오차 ≤ 5 cm + 외란 복귀 ≤ 3 s** 를
만족하는 첫(=가장 큰) `s` 를 채택한다. 결과표는 `capability._LAT_POS_ANCHORS`
(`sync_delay_anchors.py` 가 MATLAB 결과에서 파이썬·C++ 양쪽을 생성한다 — 손으로 옮기지 말 것).

**기본표 (외란 없음)** — 상위에 늘 내보내는 값

| τ [ms] | 0 | 20 | 40 | 60 | 80 | 120 | 160 |
|---|---|---|---|---|---|---|---|
| s_max | 1.00 | 1.00 | **0.88** | **0.75** | **0.37** | 운용 불가 | 운용 불가 |
| v [m/s] | 1.600 | 1.600 | 1.408 | 1.200 | 0.592 | — | — |

**돌풍표 (0.3 N·m × 0.3 s)** — 외란이 감지될 때만, rho 로 기본표와 보간

| τ [ms] | 0 | 20 | 30 | 40 | 60 | 80 |
|---|---|---|---|---|---|---|
| s_max | 1.00 | 1.00 | 1.00 | **0.55** | **0.28** | 운용 불가 |
| v [m/s] | 1.600 | 1.600 | 1.600 | 0.880 | 0.448 | — |
| 그때 복귀 [s] | 1.73 | 1.72 | 2.15 | 1.14 | 1.98 | (s=0.20 에서도 18.8) |

**두 표의 차이가 곧 "외란까지 섞어 재면 얼마나 과감쇄되는가"** 다 — 60 ms 에서
v 1.20 vs 0.45 로 2.7배. 기본표에 돌풍을 섞고 있던 초판이 실제로 크게 틀렸다.

**핵심**: τ=40 ms 에서 전속(v 1.6)은 복귀 12.9 s 로 사실상 실패인데, s=0.55 로 깎으면
종단 0.00 cm / 복귀 1.14 s — **무지연(1.73 s)보다도 낫다.** 지연이 있어도 스펙을 깎으면
외란 강건성이 유지된다는 요구가 성립한 지점이다. 80 ms 는 s=0.20 까지 내려도 안 잡혀
**운용 불가**로 표기한다 (표에 0.00 이 있으면 `mission_allowed: false`).

**탐색은 내림차순 전수여야 한다.** 복귀 시간이 배율에 단조롭지 않다 — τ=60 ms 에서
s 1.00/0.75/0.55/0.40/0.28 → 복귀 6.17/12.19/11.06/7.58/1.98 s. 이분 탐색을 쓰면
통과 구간을 건너뛴다.

**복귀 게이트는 질량에 따라 다시 잡아야 한다.** 3 s 는 1 kg 무지연 기준(1.73 s)의 약 2배다.
0 kg 은 같은 0.3 N·m 에 6배 크게 밀리므로(횡이탈 25.6 cm, 복귀 9.05 s) 절대값 3 s 를
그대로 쓰면 무지연에서조차 탈락한다 (`SPEC_GATE_REC` 로 넘긴다).

> **판정 기준을 한 번 조였다.** 처음에는 "복귀가 존재하는가"만 봤는데, 그러면
> τ=80 ms / s=0.55 가 **복귀 14.0 s** 로 통과한다. 종단오차는 4 cm 로 멀쩡했다 —
> 목표엔 도착하지만 진동이 안 잦아드는 상태다. 강건성 유지가 목적이므로
> 복귀 시간 게이트(`SPEC_GATE_REC`, 기본 3 s)가 있어야 한다. 무지연 기준값은 1.7 s.

### 두 상한이 있고, 잰 쪽이 이긴다

| 상한 | 출처 | 성격 |
|---|---|---|
| 추종 예산 `v·τ ≤ 0.5·track` | 해석 (`capability.latency_track_scale`) | **과잉 보수적** |
| 실측표 | MATLAB | 잰 구간의 진실 |

해석 규칙은 실제보다 약 5배 깎는다 — 위치 지연 60 ms, v=1.6 m/s 로 3 m 를 날았을 때
종단오차가 1.03 cm 였는데, 해석 규칙은 같은 조건에서 v 를 0.33 m/s 로 낮추라고 한다.
기준 궤적 피드포워드가 있어 측정 지연이 곧바로 추종 오차가 되지 않기 때문이다.
그래서 **잰 구간에서는 실측이 이기고, 표 밖에서만 해석 규칙을 보수적 대체값으로 쓴다**
(`degraded.latency_extrapolated` 로 표시).

### 기본은 외란 없음 — 표가 두 벌인 이유 (사용자 규정 2026-08-23)

> "디폴트는 외란 없다. 만약 시간 지연에서 외란 생기면 그때 깎는 거."

상위에 **늘** 내보내는 스펙은 지연만 반영해야 한다. 기본표에 돌풍을 섞으면 평시에도
최악을 가정한 값이 나가 임무가 필요 이상으로 느려진다.

| 표 | 조건 | 판정 | 언제 쓰나 |
|---|---|---|---|
| `_LAT_POS_ANCHORS` | 외란 없음 | 종단 ≤5 cm AND 추종 이탈 ≤10 cm | 항상 (기본) |
| `_LAT_POS_ANCHORS_GUST` | 이동 중 0.3 N·m × 0.3 s | 위 + **외란 복귀 ≤3 s** | 외란 감지 시 |

두 표는 **곱하거나 min 하지 않고 보간**한다 — `u = rho_eff / GUST_RHO_REF` 로
`s = s_nominal + (s_gust − s_nominal)·u`. 돌풍 표는 rho ≈ 0.9(권한 포화)에 해당하는
**한 점**에서 잰 것이라, 약한 돌풍에 그 값을 그대로 쓰면 과감쇄다.

### 합치는 규칙은 min 이 아니라 **가산** (사용자 지적 2026-08-23)

> "min 으로 깎는 게 아니라 + 로 해서 깎아야 하는 거 아님?"
> "자원을 1. 속도 2. 외란 3. 시간지연 얘네들이 나눠 갖는 모델인 거"

맞다. 모델은 **여유(제어 권한·위상 여유)라는 하나의 자원을 셋이 나눠 쓰는 것**이다.
배율 `s` 가 아니라 **깎인 양 `d = 1−s`** 가 소모량이므로:

```
s = 1 − Σ(1 − s_i)          # capability.combine_scales
```

`min` 은 "제일 많이 먹은 놈만 센다"는 뜻이라 **두 원인이 같은 제약을 다르게 표현한
경우에만** 옳다. 서로 다른 소비자일 때는 낙관적이다.

**예외 하나** — 지연 × 외란(위치축)은 **조합을 직접 쟀다**(위 두 표). 잰 것을 또 더하면
이중 계산이므로 거기는 표 보간을 쓴다. 안 잰 조합(자세 게이트 등)에만 가산을 적용한다.

솔직히 적어 둘 것: rho 감쇄와 돌풍 표는 펄스 응답 부분이 **일부 겹친다**(둘 다 rho 에
반응). 그래도 더하는 쪽을 택한다 — ① 두 축이 보는 외란의 모양이 다르고(rho = 지속
외란/yaw 이탈, 표 = 0.3 s 펄스) ② 겹치는 만큼 보수적으로 틀리는 편이 낫다.

**결과로 한계가 0 이 될 수 있다.** 셋이 여유를 다 먹으면 속도 몫이 없다는 뜻이고,
그게 물리적으로 옳은 답이다. 바닥으로 억지로 올리면 **없는 여유를 있다고 보고**하게
된다. 상위는 `limits.v == 0` 을 "이동 계획을 내지 말라"로 읽는다.

### 표는 시작점일 뿐 — 폐루프 교정 (`recovery_watcher.py`, 2026-08-23)

> "스펙 나누는 거 그것도 제어 루프 돌리는 거 적당히 보다가 만약 회복이 너무 더디면
> 스펙 깎고 그러는 방식으로" (사용자 설계)

실측표는 특정 조합에서 잰 것이라 실제 운용이 그 격자 위에 놓일 이유가 없다.
그래서 **표 = 피드포워드, 회복 감시 = 피드백** — 지연을 예측(부하 모델)+실측 둘 다로
잡은 것과 같은 구조다.

관측량은 **밴드 초과 지속시간**: `|측정−기준|` 이 `budget.track` 를 연속으로 넘고 있는
시간. 비행 중에는 "외란이 끝났다"는 이벤트가 없으므로, MATLAB 게이트로 쓴 복귀 시간의
온라인 대응물이 이것이다. `budget.settle` 을 넘으면 초과분에 비례해 깎고(한 판단당
최대 0.25), 잦아들면 확인 후 천천히 되돌린다.

안전장치 둘 — 둘 다 없으면 이 루프가 스스로 사고를 낸다:

1. **판단 주기 > 다리 수렴 시간.** 감쇄를 결정한 뒤에도 새 한계 안으로 드는 데
   0.66~3.94 s 가 걸린다(§5d 실측). 그보다 짧게 판단하면 앞 결정이 반영되기 전에
   또 결정하게 되고, 그게 발진이다. `BridgePlan.compliant_after_s` 를 되먹인다.
2. **기준이 한계 안일 때만 계상.** 상위가 한계를 넘는 기준을 주면 추종 오차가
   커지는데 그건 계획 문제지 스펙 문제가 아니다. 그걸로 깎으면 잘못된 계획이 기체
   능력을 갉아먹는 되먹임이 생긴다.

바닥은 0.15 이지 0 이 아니다 — **정지 판단은 감시의 권한이 아니다**(§9 감독자 몫).
감시가 낸 교정량은 `observed.recovery` 로 노출되며, 누적하면 **표를 고칠 근거**가 된다.

### 생산자: `spec_governor.py`

`capability.json` 을 실제로 쓰는 **유일한 프로세스**. 한 틱에서
관측(지연 표본·연산 부하·rho) → 융합 → 환산 → 발행. 시계 배율이 `REPUBLISH_EPS`(0.05)
이상 달라져야 상위에 알린다 — 매 변동마다 재계획하면 그게 부하를 올려 지연을 올리는
양의 되먹임이 된다.

C++ 대응물은 `qc::LatencyTracker` + `qc::SpecLatencyRule` + `qc::qc_spec_report`.
두 구현이 갈라지지 않게 `tests/test_cpp_spec_parity.py` 가 1e-9 로 대조한다
(제어 본체를 골든 트레이스로 대조하는 것과 같은 규율 — Simulink 에 대응물이 없는
경로라 파이썬이 정답 역할을 한다). 제어 출력에는 영향이 없다 (관측 전용).

---

## 5d. 재계획 인터벌 다리 (`traj_bridge.py`)

`replan_splice` 는 **τ 시점까지 옛 궤적이 유효하다**고 가정한다. waypoint 만 바뀌면 맞다.
그런데 지연/외란으로 **한계 자체를 깎는** 경우엔 틀린다 — 깎기로 결정한 순간부터 옛
궤적은 이미 새 한계를 넘는 물건이고, 계획기가 도는 동안 드론은 그걸 계속 따라간다.
스펙을 깎은 이유가 바로 그걸 막으려던 것인데.

### 구성 — 기하는 그대로, 시계만 늦춘다

`dτ/dt = s(t)`, `p(t) = p_ref(τ(t))`, `s` 는 7차 스무드스텝으로 하강(양끝에서 ṡ=s̈=s⃛=0).

1. **즉시 계산된다** — 스플라인 평가뿐 (실측 25~90 ms). 다리가 계획보다 느리면 의미가 없다.
2. **금지구역 재검사 불필요** — 지나는 점의 집합이 옛 궤적과 같다.
3. **상위가 규칙을 하나만 외운다** — `degraded.time_scale` 과 같은 대수.

### 정직하게 적어 두는 한계 — 감쇄는 **선행**이어야 한다

램프 초반에는 새 한계를 넘는다. v=1.6 m/s 로 날고 있는데 새 한계가 0.8 m/s 라면 그
자리에서 순간이동할 방법이 없다. 실측(1 kg, 예산 0.25 s):

| 새 v [m/s] | s* | 램프 [s] | 초과 v | 초과 a | 물리 한계 사용 | 새 한계 진입 [s] |
|---|---|---|---|---|---|---|
| 1.20 | 0.73 | 1.07 | 1.33× | 1.18× | 0.80 | 0.66 |
| 0.80 | 0.48 | 2.21 | 2.00× | 2.45× | 0.80 | 1.68 |
| 0.48 | 0.29 | 4.00 | 3.33× | 5.44× | 0.80 | 3.17 |
| 0.29 | 0.17 | 4.00 | 5.56× | 15.7× | 0.80 | 3.94 |

**물리 한계는 어떤 감쇄폭에서도 안 넘는다**(≤0.80) — 다리는 항상 실제로 날 수 있다.
하지만 **새 한계 안으로 들어가는 데 0.66~3.94 s** 가 걸린다. 재계획 예산(0.25 s)보다
언제나 크다. 그래서

- 지연을 **재고 나서** 깎으면 이미 늦다 → `compute_load` 의 예측(모델) 경로가 필요하다
- 인계 시점의 상태는 여전히 새 한계 밖이다 → 새 계획은 `BridgePlan.planner_limits`
  (인계 상태를 담도록 푼 한계)로 만들어야 한다. 계획기는 `v(0)=v0` 를 고정 초기조건으로
  받으므로, `v0 > v_max` 인 한계로는 **어떤 T 로도** 실행 가능한 세그먼트를 못 만든다.

### 계획이 끝내 안 오면

`replan_budget_s × 1.5` 를 지나면 `s → 0` 으로 마저 내려 **경로 위에서** 정지하고 래치한다.
옆으로 새지 않으므로 금지구역 판정이 그대로 유효하다 (`traj_emergency` A-1 과 같은 종점 규약).
