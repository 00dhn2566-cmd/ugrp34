...
`max(예산×1.5, 램프길이)` 를 지나면 `s → 0` 으로 마저 내려 **경로 위에서** 정지하고
래치한다. 옆으로 새지 않으므로 금지구역 판정이 그대로 유효하다
(`traj_emergency` A-1 과 같은 종점 규약).

### ★ 다리는 외란 강건성 수단이 **아니다** (플랜트 검증 2026-08-23)

지연 40 ms, 감쇄 직후 0.3 N·m 펄스에서 전속 유지(A) vs 다리(B):

| | 종단[cm] | 오버[cm] | 외란 횡이탈[cm] | 복귀[s] |
|---|---|---|---|---|
| A 전속 유지 | 0.18 | 6.66 | **4.03** | 9.93 |
| B 다리 | 0.14 | **1.87** | 5.45 | 9.92 |

횡이탈이 오히려 **+35% 나빴다**. 외란은 펄스가 들어온 순간의 자세 권한이 결정하는데
다리는 그때 아직 램프 중이고(새 한계 진입까지 1.43 s), 램프 중에는 ṡ 항이 추가 감속을
만들어 권한을 **더** 쓴다.

**다리에 대해 주장할 수 있는 것은 이것뿐이다**: 재계획 인터벌 동안 물리적으로 실행
가능한 기준을 주고, 기하를 안 바꿔 금지구역 재검사를 생략하게 하고, 감속하므로
오버슈트·종단이 좋아진다(-72%). **외란 대응은 rho 조속기와 돌풍 표의 몫이다.**

---

## 5e. 사용 전력량 (`energy.py`) — 추정치 → 실측 → 피드백

사용자 요구 (2026-08-26): *"먼저 사용 전력 추정치 → 실제 전력 사용량 확인 이렇게해서
피드백 넣는거"*, *"위에서 남은 전력 가지고 잘 짜지"*.

### 누가 무엇을 아는가 (역할 분리)

| 값 | 출처 | 왜 거기서 오나 |
|---|---|---|
| **남은 양** `remaining_wh` | BMS / 전력 모듈 | 잔량은 **BMS 만** 안다 (아래) |
| **쓸 양** `energy_wh` | `energy.py::estimate_energy` | 궤적만 있으면 **비행 전에** 나온다 |
| **판정** `affordable` | 계획기 | 위 둘을 비교 |
| **교정** `ratio_act_over_est` | 비행 후 실측 ÷ 추정 | 추정기의 효율곱을 고친다 |

**잔량을 모터 관측으로 구하면 안 된다.** 모터 회전량·부하가 주는 것은 "지금 얼마나
쓰고 있나"(순간 전력)이지 "얼마나 남았나"가 아니다. 모터에서 잔량을 얻으려면 만충
시점부터 적분해야 하는데 ① 적분 드리프트를 되잡을 기준점이 없고 ② 배터리 노화로 준
실용량을 못 보고 ③ 온도·내부저항·전압 강하에 따른 가용 에너지 감소를 못 본다. 즉
**열린 루프**다. BMS(또는 전압+전류 센서 + 쿨롱 카운팅)는 전압으로 되잡을 수 있는
**닫힌 루프**다.

모터 관측(`power_from_motors`)은 대신 BMS 가 못 하는 둘을 한다 — **교차검증**(모터
유도 전력과 BMS 실측이 벌어지면 프로펠러 손상·모터 이상·BMS 오교정 중 하나)과
**BMS 없는 시뮬에서의 대체 실측**.

실기 주의: FX450 급에 스마트 BMS 가 달릴 가능성은 낮다. 현실적 최소 구성은 **전력
모듈(전압+전류)** 이고 거기서 쿨롱 카운팅 + 전압 기반 SoC 보정을 한다.

### 추정식 (운동량 이론)

쿼드는 추력이 기체 z 축을 향하므로 미분평탄성으로 `T = m·|a − g|` 이다. 기울기
보정을 따로 안 해도 되는 이유가 이것이다 (`m(g+z̈)/(cosφcosθ)` 와 같은 식).

```
T_tot   = m·|a − g|
P_ideal = n_rot · (T_tot/n_rot)^1.5 / sqrt(2·rho·A)      A = pi·R_prop²
P       = P_ideal / (FM·eta)                             FM=0.70, eta=0.80 (미측정)
E       = integral(P dt)
```

눈금: 1 kg 짐(m_tot 2.2726 kg) 호버 **267 W = 8.5 g/W** — 소형 멀티로터 실측 대역.

**FM·eta 는 아직 안 잰 값이다.** 그래서 추정치는 `calibrated:false` 를 달고 다니고,
계획기는 미교정 추정치에 1.3배 안전계수를 얹어 판정한다 (`EnergyBudget.can_afford`).

### 세 모델이 갈린다 — 그래서 실측 교정이 필요하다 (실측 2026-08-26)

같은 호버(로터 4개 634 rad/s, 추력 22.3 N)를 세 모델이 이렇게 예측했다:

| 출처 | 호버 전력 | 효율 | 판단 |
|---|---|---|---|
| 운동량 이론 (`energy.py`) | 267 W | 8.5 g/W | 실기 대역과 일치 |
| Simscape 배터리 (`verify_worstcase.m`) | 90 W | 25 g/W | **불가능하게 좋다** |
| `qc_motor.hpp` 의 Cq | 636 W | 3.6 g/W | **비현실적으로 나쁘다** |

- Simscape 배터리는 SOC 표시용 이상화 모델로 보인다 (모터 동손·ESC 손실 미포함).
- `qc_motor.hpp` 의 `Cq` 는 프로펠러 공력이 아니라 **토크 클램프 0.2 N·m 에서
  634 rad/s 가 되도록 역산**된 값이다 (그 파일 주석 참조). 공력 계수가 아니다.

⚠ **이 둘로는 교정하지 않는다** (`energy.TRUSTED_SOURCES`). 낙관적인 쪽(0.335배)으로
교정하면 계획기가 여유를 3배로 착각하고, 그건 배터리가 임무 중에 죽는 방식이다.
교정은 **실기 전력 모듈**이 들어온 뒤에 연다. 그때까지 시뮬 실측은 `trusted:false`
로 기록만 한다.

### 피드백 파일 (`output/energy_feedback.json`)

```json
{
  "schema_version": "0.1",
  "source": "power_module",          // bms | power_module | flight_log 만 교정에 쓰인다
  "trusted": true,
  "n_samples": 9,
  "ratio_act_over_est": 1.08
}
```

가드레일은 6절(플랜트 상수 추정 소비)과 같은 원칙이다: **앵커 불변**(기체 상수는
안 건드리고 교정은 배수로만), **램프**(1회 ±20%), **표본 하한**(3개 미만이면 미반영),
**클램프**(0.30~3.00 밖은 측정 오류로 본다).

### 상위 보고 블록 (`energy_block()`)

```json
{
  "estimate": {"energy_wh": 1.334, "energy_wh_per_m": 0.445, "power_mean_w": 267.0,
               "power_peak_w": 289.1, "duration_s": 17.95, "distance_m": 3.0,
               "calibrated": false, "efficiency": 0.56},
  "budget":   {"remaining_wh": 50.0, "usable_wh": 40.0, "reserve_frac": 0.2,
               "source": "bms", "trusted": true},
  "affordable": true, "headroom_wh": 38.27, "repeats_possible": 23
}
```

`repeats_possible` 는 "남은 전력으로 이 임무를 몇 번 더 할 수 있나" — RL 보상과 임무
선택에 바로 쓰이는 단위다.

### 스펙을 깎으면 에너지는 **늘어난다**

이 드론에서 에너지는 사실상 `무게 x 시간` 이다. 3 m 이동 케이스의 평균 전력이
267 W 로 **호버 전력과 같다** — 기동 자체가 더 먹는 몫은 측정 한계 안이다. 게인을
조이는 것도 마찬가지로 무시할 만하다 (호버 지터가 0.002°라 여분의 움직임이 없다).

그래서 정확도/강건성의 에너지 대가는 게인이 아니라 **시간**에서 나온다: 지연 때문에
스펙 배율 `s` 를 깎으면 이동시간이 `1/s` 로 늘고 에너지도 그만큼 는다. 5c 절의 감쇄를
채택할 때 이 비용을 같이 봐야 한다 — 실측은 `figure/13_worstcase/fig_worst_energy.png`.
