...
- **무명령 기본 정책 (사용자 확정 2026-07-17)**: 상위가 침묵하면 드론은
  **마지막 유효 setpoint를 유지하며 호버**한다. 구현 근거: ① 궤적은 스트리밍이
  아니라 전체 시간표 선적재 — 비행 중 통신 두절 시 현행 궤적을 끝까지 비행
  ② 궤적 종료 후 기준은 종점에 클램프(Simulink Lookup Table 외삽 동작; tail
  hold 8s 잔류 0.000° 검증이 이 상태의 실측) ③ 새 미션 거부 시 기존 궤적
  유효 유지. **C++ 이식 시 이 클램프 외삽 동작을 반드시 재현할 것** — 이게
  빠지면 무명령 default가 조용히 사라진다 (종점 이후 기준 미정의 = 발산 위험).
