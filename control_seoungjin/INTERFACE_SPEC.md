...
- **스캔 속도의 소유권 (사용자 지적 2026-07-19: "얼마나 빨리 스캔할지 제어는
  ...
  프레임당 ~6° 회전 — 겹침 충분. 요청값 산정은 비전 쪽 몫. **기본값은 "스캔 안
  함"** — yaw 블록 자체가 선택이고 기본 mode=heading.
- **스캔↔시간 배분 결합 규칙 (사용자 지적 2026-07-19)**: 스캔은 자기 소요시간
  (구간각/rate)이 있는 명령이라 이동 시간표와 결합된다.
  **미션 소요시간 = max(이동 시간, 스캔 시간).**
  ① 스캔이 더 길면 → 이동 시간표를 균일 팽창해 스캔이 이동 중 완료되게 맞춤
  (종점 호버 대기보다 부드러움) — 팽창은 §7 `TIME_DILATED` adjustment로 통지
  (재시간화와 동일 통로, reason: scan). ② 이동이 더 길면 → 스캔은 요청 속도
  그대로 (rate는 비전의 제약 — 임의 감속 금지), 완료 후 마지막 각도 유지.
  ③ 스캔 시간이 지배하는 미션은 위치 한계 여유가 남아돎 — margins에 그대로
  드러나므로 상위가 "더 빨리 날 수 있었다"를 학습 가능.
