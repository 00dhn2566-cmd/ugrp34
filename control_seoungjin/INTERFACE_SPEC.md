...
- **스캐닝은 각도 명령이 아니라 `scan` 프리미티브** (사용자 용도 확인 2026-07-19:
  ...
  지휘하지 않는다 (waypoint에 시간을 안 붙이는 것과 동일 철학). 스캔 속도 상한
  결정 요인 2개: ① yaw 권한(토크 포화) ② **비전 프레임 겹침 — 카메라 FOV·탐지
  주기와 함께 길남 파트와 확정 필요 (조율점, 잠정 1.0 rad/s면 FOV 90°/10Hz에서
  프레임당 ~6° 회전으로 충분 겹침)**. 이동 중 스캔 허용 (yaw 독립 DOF).
