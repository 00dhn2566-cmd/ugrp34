...
- [ ] **counter_swing 2호기 — 필수로 승격** (사용자 역할 확정: 외란 유발 스윙 감쇠도 궤적 층 소관). 3단계: ① 교정 비행 → 자세°↔가속 상수 (`diagnose_swing_calib.m`) ② tail 기반 오프라인 연결(다음 비행에 반영 — 현 인프라로 가능) ③ 비행 중 실시간: 스윙 관측기(자세 잔차 1~3Hz 대역 추출) + delta 실시간 주입(스플라이스 통로 재사용)
