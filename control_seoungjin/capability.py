...
#
#   ⚠ 다만 실물은 질량에 **단조롭지 않다**. 0.5 kg 실측 (2026-08-28):
#         tau     0/20    40      60      80
#         0   kg  1.00    1.00    0.83    0.75
#         0.5 kg  1.00    1.00    0.92 ←  0.75      (60 ms 에서 0 kg 보다 좋다)
#         1   kg  1.00    0.88    0.75    0.37
#     선형 보간의 예측은 (—, 0.94, 0.79, 0.56) 으로 **네 점 모두 실측보다 낮다**.
#     즉 보간은 틀리되 늘 '느리게 가라' 는 쪽으로 틀린다. 그래서 지금 쓰기에는
#     안전하지만, "무거울수록 지연에 약하다" 를 물리 법칙처럼 믿으면 안 된다.
#     (`test_monotone_in_mass` 는 **보간표의 성질**을 검사하는 것이지 실물이 아니다.)
#
#   ⚠ 현실 대역에서는 이 표가 거의 잠들어 있다. 위치 경로 30 ms 이하에서는 세 질량
#     모두 1.00 이다 (펌웨어 요구사항의 목표 대역). 이 표의 값어치는 VIO 가 예상보다
#     느릴 때의 안전망이지, 평시 임무 속도를 정하는 물건이 아니다.
#
