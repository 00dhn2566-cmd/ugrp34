...
def _parse_ts(s):
    """ISO 유사(콜론→하이픈) 타임스탬프 파싱. 소수초는 무시."""
    return datetime.strptime(str(s).split(".")[0], TS_FMT)
...
def load_mission(path):
    ...
    스키마 (sample/INPUT_FORMAT.md 확장):
        waypoints : [[x,y,z], ...]  (필수, N>=2)
        limits    : {v_max, a_max, j_max, snap_max}  (필수, 숫자 또는 [x,y,z])
        dt        : 샘플 간격 [s] (선택, 기본 0.01)
        shaper    : {mode: 'zv'|'zvd', f_mode_hz} (선택)
    """
    ...
    for key in ("waypoints", "limits"):
        if key not in cfg:
            raise KeyError(f"경로 JSON에 필수 키 '{key}' 없음: {path}")
...
def build_trajectory(cfg, waypoints, f_mode):
    ...
    # 1) 시간 부여 (snap까지 제약하는 최소시간 7차 다항식 — 저크-가능 조건 내장)
