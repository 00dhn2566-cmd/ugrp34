...
cfgs = { '1 기준(kd-150,fD1000)',      -150, 1000, false; ...
         '6 kd-200',                   -200, 1000, false; ...
         '7 kd-150+fD2000',            -150, 2000, false; ...
         '8 kd-200+fD2000',            -200, 2000, false; ...
         '9 kd-250+fD2000',            -250, 2000, false };
results = {};
posCutDone = false;
fprintf('\n===== roll/pitch 지터 원인 분리 (5구성 x %gs 호버) =====\n', T);
for ci = 1:size(cfgs, 1)
    tag = cfgs{ci,1};
    kd_attitude = cfgs{ci,2};
    filtD_attitude = cfgs{ci,3};
    needCut = cfgs{ci,4};
    ...
    fprintf('\n--- 구성 %s: kd=%g filtD=%g ---\n', tag, kd_attitude, filtD_attitude);
    try
        sim(mdl);
    catch e
        fprintf('  시뮬 실패: %s\n', e.message);
