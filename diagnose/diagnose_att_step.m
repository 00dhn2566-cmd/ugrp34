...
% --- 자세 루프 스텝 테스트: 위치 루프 절단 + 고정 cmd 주입 ---
% PC Out1/Out2 (roll/pitch cmd 경로) 라인을 끊고 상수로 대체:
% 한쪽에 +5도, 다른쪽 0 -> 로그의 cmd_roll/cmd_pitch가 어느 채널인지 자동 식별.
% 판정: real이 +5도로 수렴 = 자세 루프 부호 정상 / 반대로 가면 반전 확정.
...
for oi = 1:2
    ...
    if l == -1
        error('PC Out%d 라인 없음 - 배선이 예상과 다름, 실행 무효', oi);
    end
    dsts = get_param(l, 'DstPortHandle');
    for dd = dsts(:)'
        fprintf('PC Out%d 원래 목적지: %s (in%s)\n', oi, ...
            strrep(get_param(dd,'Parent'), newline, '|'), num2str(get_param(dd,'PortNumber')));
    end
    delete_line(l);
...
try
    ...
    for ct = [0.1 0.25 0.5 1.0 1.5 2.0]
        [~, idx] = min(abs(t - ct));
    ...
    fprintf('\n=== 스텝 응답: cmd 자세 vs 실제 자세 (한 채널에 +5도 주입) ===\n');
