...
% --- cmd 크기 원천 제한: Pitch/Roll Limit ±60 -> ±10도, x,y sat 0.15 -> 0.05 ---
pcSat = [mdl '/Maneuver Controller/Position Control'];
set_param([pcSat '/Pitch Limit'], 'UpperLimit', 'pi/18', 'LowerLimit', '-pi/18');
set_param([pcSat '/Roll Limit'],  'UpperLimit', 'pi/18', 'LowerLimit', '-pi/18');
...
if isempty(find_system(pc, 'SearchDepth', 1, 'Name', 'PosErr Sat X'))
    add_block('simulink/Discontinuities/Saturation', satXBlk, 'UpperLimit', '0.05', 'LowerLimit', '-0.05');
end
if isempty(find_system(pc, 'SearchDepth', 1, 'Name', 'PosErr Sat Y'))
    add_block('simulink/Discontinuities/Saturation', satYBlk, 'UpperLimit', '0.05', 'LowerLimit', '-0.05');
end
...
fprintf('=== cmd 크기 원천제한 테스트 (Roll/Pitch Limit ±10도, x,y sat ±0.05m) ===\n');
try
    simOut = sim(mdl);
    t = real_z.time(:);
