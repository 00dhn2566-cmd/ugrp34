...
% --- cmd roll/pitch에 -1 곱하기 (폐루프 부호 반전 보정: 사용자 지시) ---
% roll/pitch 폐루프가 부호 반대(뒤집힌 자세가 안정 평형)임이 확인됨.
% Position Control의 Pitch Cmd/Roll Cmd 출력에 -1 게인을 삽입해
% 안정 평형점을 정상 자세로 되돌린다.
mc = [mdl '/Maneuver Controller'];
pcB = [mc '/Position Control'];
pcPh = get_param(pcB, 'PortHandles');
for oi = [1 2]  % Outport 1 = Pitch Cmd, 2 = Roll Cmd
    lineH = get_param(pcPh.Outport(oi), 'Line');
    if lineH == -1; continue; end
    dstH = get_param(lineH, 'DstPortHandle');
    delete_line(lineH);
    gName = sprintf('NegCmd%d', oi);
    gBlk = [mc '/' gName];
    if isempty(find_system(mc, 'SearchDepth', 1, 'Name', gName))
        add_block('simulink/Math Operations/Gain', gBlk, 'Gain', '-1');
    end
    gPh = get_param(gBlk, 'PortHandles');
    add_line(mc, pcPh.Outport(oi), gPh.Inport(1), 'autorouting', 'on');
    for d = 1:numel(dstH)
        add_line(mc, gPh.Outport(1), dstH(d), 'autorouting', 'on');
    end
    fprintf('%s 삽입: Position Control Outport %d -> -1 -> 원래 목적지\n', gName, oi);
end
...
fprintf('=== cmd roll/pitch 부호반전(-1) 호버 테스트 ===\n');
try
    simOut = sim(mdl);
    t = real_z.time(:);
