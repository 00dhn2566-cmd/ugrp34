...
% Roll(In1)과 Pitch(In2) 채널 부호를 4개 모터 전부 반전
% (= 믹서가 가정하는 기하를 180도 회전; FX450 CAD 교체로 프레임이
%  원본과 180도 다르게 들어갔다는 가설의 검증)
for aName = {'Add4','Add5','Add6','Add7'}
    blk = [mixer '/' aName{1}];
    ss = get_param(blk, 'Inputs');
    for k = 1:2
        if ss(k) == '+'; ss(k) = '-'; else; ss(k) = '+'; end
    end
...
fprintf('=== 기하 180도 수정(Roll/Pitch 채널 반전) 호버 테스트 ===\n');
try
    simOut = sim(mdl);
    t = real_z.time(:);
