...
% --- Revolute Joint 구속 토크 센싱 활성화 + PS-Simulink 변환 + 로깅 ---
ctNames = {};
for pp = 1:4
    psys = sprintf('%s/Quadcopter/Propeller %d', mdl, pp);
    rjL = find_system(psys, 'LookUnderMasks','all','FollowLinks','on','RegExp','on','Name','Revolute');
    rj = rjL{1};
    set_param(rj, 'SenseConstraintTorque', 'on');
    % 새로 생긴 PS 출력 포트 찾기 (기존: LConn 2개, RConn 2개였음)
    ph2 = get_param(rj, 'PortHandles');
    newPort = ph2.RConn(end);  % 센싱 포트는 마지막 RConn으로 추가됨
    convName = sprintf('CTConv%d', pp);
    convBlk = [psys '/' convName];
    if isempty(find_system(psys, 'SearchDepth', 1, 'Name', convName))
        add_block('nesl_utility/PS-Simulink Converter', convBlk);
    end
    cph = get_param(convBlk, 'PortHandles');
    add_line(psys, newPort, cph.LConn(1));
    vName = sprintf('ct%d', pp);
    twName = ['To Workspace ' vName];
    if isempty(find_system(psys, 'SearchDepth', 1, 'Name', twName))
        twB = [psys '/' twName];
        add_block('simulink/Sinks/To Workspace', twB, 'VariableName', vName, 'SaveFormat', 'StructureWithTime');
        tph = get_param(twB, 'PortHandles');
        add_line(psys, cph.Outport(1), tph.Inport(1), 'autorouting', 'on');
    end
    ctNames{end+1} = vName;
    fprintf('P%d 구속토크 센싱 연결 완료\n', pp);
end
...
fprintf('=== 조인트 구속토크 직접 측정 (회전부가 몸체에 거는 토크) ===\n');
try
    simOut = sim(mdl);
    t = real_z.time(:);
    ...
    fprintf('\n=== 조인트 구속토크 (N*m) ===\n');
    c1 = ct1.signals.values; c2 = ct2.signals.values; c3 = ct3.signals.values; c4 = ct4.signals.values;
    fprintf('  %5s | %9s %9s %9s %9s | %8s\n', 't','ct1','ct2','ct3','ct4','roll');
    for ct_ = 0:0.03:0.6
        [~, idx] = min(abs(t - ct_));
        fprintf('  %5.2f | %+9.4f %+9.4f %+9.4f %+9.4f | %8.2f\n', t(idx), c1(idx,1), c2(idx,1), c3(idx,1), c4(idx,1), r(idx));
    end
