...
% --- 회전 부품(Housing/Propeller/Cap) 관성 ~0 실험 ---
for pp = 1:4
    psys = sprintf('%s/Quadcopter/Propeller %d', mdl, pp);
    for partName = {'Housing','Propeller','Cap'}
        blkL = find_system(psys, 'SearchDepth', 1, 'Name', partName{1});
        if isempty(blkL); continue; end
        blk = blkL{1};
        try
        ...
        if contains(it, 'Custom')
            m0 = str2double(get_param(blk, 'Mass'));
            set_param(blk, 'Mass', num2str(m0*1e-6, '%.6g'));
            moi = get_param(blk, 'MomentsOfInertia');
            set_param(blk, 'MomentsOfInertia', ['(' moi ')*1e-6']);
            fprintf('P%d %s: Custom 관성 1e-6 스케일\n', pp, partName{1});
        else
            try
                set_param(blk, 'Density', '1e-6');
                fprintf('P%d %s: Density=1e-6\n', pp, partName{1});
            catch
                fprintf('P%d %s: 관성 축소 실패 (InertiaType=%s)\n', pp, partName{1}, it);
            end
        end
    end
end
...
fprintf('=== 회전부 관성 제거 실험: 발산이 사라지면 회전 관성이 범인 ===\n');
try
    simOut = sim(mdl);
    t = real_z.time(:);
