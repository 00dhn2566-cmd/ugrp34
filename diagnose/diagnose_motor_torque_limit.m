%% limit_motor(모터 출력 토크 상한)가 너무 작아서 4개 모터가 전부 포화돼
%% 같은 속도로 수렴하는지 확인 -> 크게 키워서 재테스트.
...
fprintf('원래 limit_motor=%g, qc_motor.max_torque=%g qc_motor.max_power=%g\n', ...
    limit_motor, qc_motor.max_torque, qc_motor.max_power);

limit_motor = limit_motor * 50;
fprintf('테스트 limit_motor=%g\n', limit_motor);
