...
% --- YPR 내부: Roll Cmd 인포트 -> (x -1) -> 원래 목적지(자세 PID 오차합) ---
ypr = [mdl '/Maneuver Controller/Altitude and  YPR Control'];
rcBlk = [ypr '/Roll Cmd'];
rcPh = get_param(rcBlk, 'PortHandles');
lineH = get_param(rcPh.Outport(1), 'Line');
if lineH ~= -1
    dstH = get_param(lineH, 'DstPortHandle');
    for d = 1:numel(dstH)
        fprintf('Roll Cmd 원래 목적지: %s (port %d)\n', get_param(dstH(d),'Parent'), get_param(dstH(d),'PortNumber'));
    end
    delete_line(lineH);
    gBlk = [ypr '/NegRollCmd'];
    if isempty(find_system(ypr, 'SearchDepth', 1, 'Name', 'NegRollCmd'))
        add_block('simulink/Math Operations/Gain', gBlk, 'Gain', '-1');
    end
    gPh = get_param(gBlk, 'PortHandles');
    add_line(ypr, rcPh.Outport(1), gPh.Inport(1), 'autorouting', 'on');
    for d = 1:numel(dstH)
        add_line(ypr, gPh.Outport(1), dstH(d), 'autorouting', 'on');
    end
    fprintf('NegRollCmd(-1) 삽입 완료: Roll Cmd -> -1 -> PID 오차합\n');
else
    fprintf('경고: Roll Cmd 인포트에 연결된 라인 없음!\n');
end
...
fprintf('=== Roll Cmd x(-1) -> PID 입력 직접 반전 테스트 ===\n');
try
    simOut = sim(mdl);
    t = real_z.time(:);
