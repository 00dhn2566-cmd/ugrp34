%% Pos Error(Subtract2 출력, Traj.pos - m.Chassis.p) 자체에 saturation을 걸어서
%% PID 입력으로 들어가는 오차 크기를 애초에 제한 -> anti-windup 없이도
%% PID 내부값 폭주를 원천 차단할 수 있는지 확인.
...
% Subtract2(Pos Error) 출력과 PID Controller 입력 사이에 Saturate 삽입
subBlk = [pc '/Subtract2'];
...
if isempty(find_system(pc, 'SearchDepth', 1, 'Name', 'Pos Error Saturate'))
    add_block('simulink/Discontinuities/Saturation', satBlk, 'UpperLimit', '0.5', 'LowerLimit', '-0.5');
end
...
% Subtract2 출력은 원래 PID Controller뿐 아니라 내부 Scope(Inport2)로도 분기돼
% 있었음 - 그 분기도 다시 연결해줘야 함
scopeBlk = [pc '/Scope'];
...
fprintf('=== Pos Error에 ±0.5m saturation 적용 후 베이스라인 호버 (IC=0) ===\n');
simOut = sim(mdl);
...
t = real_roll.time(:);
fprintf('\n=== cmd_roll/cmd_pitch (Pos Error ±0.5m 제한 적용 후) ===\n');
