%% Pos Error saturation을 ±0.1m로 더 타이트하게 걸어서 재확인.
...
if isempty(find_system(pc, 'SearchDepth', 1, 'Name', 'Pos Error Saturate'))
    add_block('simulink/Discontinuities/Saturation', satBlk, 'UpperLimit', '0.1', 'LowerLimit', '-0.1');
end
...
fprintf('=== Pos Error에 ±0.1m saturation 적용 후 베이스라인 호버 (IC=0) ===\n');
simOut = sim(mdl);
...
t = real_roll.time(:);
fprintf('\n=== cmd_roll/cmd_pitch (Pos Error ±0.1m 제한 적용 후) ===\n');
