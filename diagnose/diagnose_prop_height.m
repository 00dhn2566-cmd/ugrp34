...
% --- 역진자 가설 진단: 프로펠러 장착점을 +100mm 올려서 추력면을 CG 위로 ---
for pp = 1:4
    tp = sprintf('%s/Quadcopter/Propeller %d/Transform Propeller', mdl, pp);
    tpList = find_system(sprintf('%s/Quadcopter/Propeller %d', mdl, pp), 'LookUnderMasks','all','FollowLinks','on','RegExp','on','Name','Transform.Propeller');
    if ~isempty(tpList)
        oldOff = get_param(tpList{1}, 'TranslationCartesianOffset');
        set_param(tpList{1}, 'TranslationCartesianOffset', '[0 0 80]');
        fprintf('P%d Transform Propeller offset: %s -> [0 0 80] (mm)\n', pp, oldOff);
    end
end
...
fprintf('=== 역진자 가설 진단: 프롭 장착 +100mm 상향 테스트 ===\n');
try
    simOut = sim(mdl);
    t = real_z.time(:);
