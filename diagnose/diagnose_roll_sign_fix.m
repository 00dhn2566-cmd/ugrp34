%% roll 채널 부호 반전 + attitude 권한 축소 테스트:
%% - roll이 모든 세션에서 명령과 반대로(항상 -90도로만) 발산해온 것이
%%   roll 채널 피드백 부호 반전(양의 피드백) 때문이라는 가설 검증.
%%   믹서의 roll 채널(In1) 부호를 4개 모터 전부 반전.
%% - attitude 권한도 새 레짐에 맞게 축소: limit_attitude 800->50, limit_yaw 20->10
...
limit_attitude = 50;  % 800 -> 50 (새 레짐: 호버 101 rev/s 대비)
limit_yaw = 10;       % 20 -> 10
...
% --- roll 채널(In1)만 4개 모터 전부 부호 반전 (roll 양의피드백 수정) ---
for aName = {'Add4','Add5','Add6','Add7'}
    blk = [mixer '/' aName{1}];
    ss = get_param(blk, 'Inputs');
    if ss(1) == '+'; ss(1) = '-'; else; ss(1) = '+'; end
    set_param(blk, 'Inputs', ss);
    fprintf('%s roll부호 반전 후: %s\n', aName{1}, ss);
end
...
fprintf('=== roll 부호 반전 + attitude 권한 축소 호버 테스트 ===\n');
try
    simOut = sim(mdl);
    t = real_z.time(:);
