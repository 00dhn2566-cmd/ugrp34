...
% swing_calib.json 저장 - counter_swing_offset(2호기) 소비용 상수 파일.
% 소비 규약은 attitude_feedback와 동일 계열(written_at 신선도 판단), 위상은
% "이동 시작 -> 첫 양피크 지연[s]" 그대로 실음 (역위상 주입 시 f0으로 전파).
if size(res,1) >= 2
    S_all = cell2mat(res(:,5));
    f0_all = cell2mat(res(:,3));
    calib = struct( ...
        'schema_version', '0.1', ...
        'written_at', char(datetime('now','Format','yyyy-MM-dd''T''HH:mm:ss')), ...
        'f0_hz', median(f0_all(~isnan(f0_all))), ...
        'S_deg_per_m', mean(S_all), ...
        'linearity_ratio', max(S_all)/max(min(S_all), eps), ...
        'peak_delay_s', res{1,4}, ...
        'pulses', struct('amp_m', res(:,1), 'peak_deg', res(:,2), ...
                         'f0_hz', res(:,3), 'peak_delay_s', res(:,4), ...
                         'S_deg_per_m', res(:,5)), ...
        'profile_note', 'S는 이 표준 펄스(quintic, Tm 저크산정) 프로파일 기준', ...
        'used', false);
    ...
    fclose(fid);
    fprintf('[write] %s (선형성 비율 %.2f - 1.15 이하면 스케일링 신뢰)\n', ...
        outPath, calib.linearity_ratio);
else
    fprintf('[skip] 펄스 2편 미완주 - swing_calib.json 미저장 (선형성 검증 불가)\n');
end
