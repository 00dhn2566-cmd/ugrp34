...
% --- [4] 후보 3세트 실행 ---
% 최종 판정 매트릭스: kp(10/15) x ki(0/1.5). 지속외란 0.01N·m 하에서
% ① 오프셋 소거(I 효과) ② yaw 권한 점유율/모터 헤드룸(사용자 기준) ③ roll/pitch 침해
gainSets = [10 0 3; 15 0 4; 10 1.5 3; 15 1.5 4];
...
for gi = 1:size(gainSets, 1)
    ...
    % 지표 ④ 제어량 (수정판): 반대회전 부호 제거(abs) 후 쌍 차등, 외란 전 기준 대비 변화량으로 평가
    Wa = abs(W);
    d14 = (Wa(:,1) + Wa(:,4)) - (Wa(:,2) + Wa(:,3));
    d13 = (Wa(:,1) + Wa(:,3)) - (Wa(:,2) + Wa(:,4));
    mPre = t > 5 & t < 8;    % 외란 전 (스핀업 제외)
    mPost = t > 12;          % 외란 정착 후
    sc14 = std(d14(mPost)) + abs(mean(d14(mPost)) - mean(d14(mPre)));
    sc13 = std(d13(mPost)) + abs(mean(d13(mPost)) - mean(d13(mPre)));
    if sc14 >= sc13; dY = d14; pairTag = '(1+4)-(2+3)'; else; dY = d13; pairTag = '(1+3)-(2+4)'; end
    baseD = mean(dY(mPre));
    eff = dY - baseD;
    wHover = mean(Wa(mPre, :), 'all');
    effSS = mean(abs(eff(mPost)));              % 정상상태 yaw 노력 (외란 상쇄 유지분)
    effPk = max(abs(eff(t > 2)));               % 피크 노력
    authSS = 100 * effSS / (2 * limit_yaw);     % yaw 권한(2 x limit_yaw) 점유율
    authPk = 100 * effPk / (2 * limit_yaw);
    jit = std(diff(dY(t > 2)));
    wDev = max(abs(Wa(t > 2, :) - mean(Wa(mPre, :), 1)), [], 1);  % 모터별 호버 대비 최대 이탈
    mask2 = t > 2;
    fprintf('  >> 초기 최대|yaw| %.2f도 / 외란 이탈 %.2f도 / 복귀 %.2fs / 자세RMS %.2f / z [%.2f %.2f]\n', ...
        maxIni, dev, tRec, sqrt(mean(r(mask2).^2 + pch(mask2).^2)), min(zv), max(zv));
    fprintf('  >> yaw 노력 %s: 정상 %.2f rev/s (권한의 %.1f%%) / 피크 %.2f rev/s (권한의 %.1f%%) / 지터 %.3f\n', ...
        pairTag, effSS, authSS, effPk, authPk, jit);
    fprintf('  >> 모터별 호버(%.1f rev/s) 대비 최대 이탈: [%.2f %.2f %.2f %.2f] rev/s\n', wHover, wDev);
    if authPk > 80
        fprintf('  ⚠ yaw가 권한의 80%% 초과 점유 - roll/pitch 헤드룸 침해, 탈락 후보\n');
    end
end
