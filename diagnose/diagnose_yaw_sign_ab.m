%% yaw 부호 A/B 테스트: thrust_path_rescale과 동일 조건에서 yaw 게인 부호만 반전.
%% yaw 단조발산(-45도@0.25s)이 부호 오류인지 확인.
%%
...
kp_yaw      = -3;   ki_yaw = 0;         kd_yaw = -1;  % yaw 부호 A/B: 반전
...
fprintf('=== yaw 부호 반전 A/B 테스트 (kp_yaw=-3, kd_yaw=-1, 나머지 동일) ===\n');
try
    simOut = sim(mdl);
    t = real_z.time(:);
