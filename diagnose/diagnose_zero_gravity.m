...
% --- 중력 OFF: 외란이 '돌아간 CG x 중력' 모멘트인지 확정 ---
% 이름 검색은 두 번 실패(블록명 불일치) → 전 블록을 훑어 Gravity 파라미터 보유 블록을 직접 찾는다.
...
for i = 1:numel(allBlk)
    try
        ...
        if isstruct(dp)
            fn = lower(fieldnames(dp));
            if any(~cellfun('isempty', strfind(fn, 'gravity')))
                gravBlk{end+1} = allBlk{i}; %#ok<AGROW>
            end
        end
    catch
    end
end
fprintf('Gravity 파라미터 보유 블록: %d개\n', numel(gravBlk));
for i = 1:numel(gravBlk)
    fprintf('  [%d] %s\n', i, strrep(gravBlk{i}, newline, '|'));
    try
        fprintf('      UniformGravity(전) = %s\n', get_param(gravBlk{i}, 'UniformGravity'));
    catch
    end
    try
        fprintf('      Gravity(전) = %s\n', get_param(gravBlk{i}, 'Gravity'));
    catch
    end
    gset = false;
    try
        set_param(gravBlk{i}, 'UniformGravity', 'None');
        gset = true;
        fprintf('      -> UniformGravity=None 설정 완료\n');
    catch
    end
    if ~gset
        try
            set_param(gravBlk{i}, 'Gravity', '[0 0 0]');
            gset = true;
            fprintf('      -> Gravity=[0 0 0] 설정 완료\n');
        catch e2
            fprintf('      설정 실패: %s\n', e2.message(1:min(160,end)));
        end
    end
end
if isempty(gravBlk)
    error('무중력 설정 실패: Gravity 파라미터 블록을 하나도 못 찾음 - 실행 무효, 표가 나와도 믿지 말 것');
end
...
fprintf('=== 무중력 실험: 발산이 사라지면 [돌아간 CG x 중력] 확정 ===\n');
try
    simOut = sim(mdl);
    t = real_z.time(:);
