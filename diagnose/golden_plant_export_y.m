...
% --- 시나리오: 1 m 젠틀무브 Y축 (롤 여기용) ---
VMAX = 2.0; AMAX = 2.0; JMAX = 10.0;
dt = 0.01; T = 12; tStep = 3; Amp = 1.0;
N = round(T/dt) + 1;
tt = (0:N-1)' * dt;
tau = min(max((tt - tStep)/0.9, 0), 1);
yk = Amp * (10*tau.^3 - 15*tau.^4 + 6*tau.^5);
sm = traj_smoother(tt, [zeros(N,1), yk, ones(N,1)], VMAX, AMAX, JMAX);
waypoints = [0 0 1; 0 Amp 1]';
...
outFile = fullfile(csvDir, 'golden_plant_trace_y.csv');
