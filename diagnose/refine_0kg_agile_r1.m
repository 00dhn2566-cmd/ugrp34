%% agile 전용 0kg 앵커 좌표하강 r1 (18차, 사용자 지시): agile 위치 게인(24/10.8) 아래서
%% 자세 x 고도 배율 격자 - agile 전용 1차식의 g0 앵커 확보.
%% 배경: 표준 1차식(0.75/0.56, precision 하 실측)을 agile에 적용하니 0.5kg 발산 +
%%       2kg z피크 85cm (verify_agile_gates). 위치 kp가 z축에도 걸려 앵커가 프로파일 의존.
...
cfgs = { ...
    'sA0.55 sZ0.40', 0.55, 0.40; ...
    'sA0.55 sZ0.56', 0.55, 0.56; ...
    'sA0.55 sZ0.72', 0.55, 0.72; ...
    'sA0.70 sZ0.40', 0.70, 0.40; ...
    'sA0.70 sZ0.56', 0.70, 0.56; ...
    'sA0.70 sZ0.72', 0.70, 0.72; ...
    'sA0.85 sZ0.40', 0.85, 0.40; ...
    'sA0.85 sZ0.56', 0.85, 0.56; ...
    'sA0.85 sZ0.72', 0.85, 0.72; ...
};
...
nC = size(cfgs,1);
rows = nan(nC, 10);
fprintf('===== agile 전용 0kg 앵커 r1: 위치 24/10.8 고정, 자세 x 고도 배율 (질량 0, 이동 1m 젠틀무브 0.9s) =====\n');
...
writetable(Tb, fullfile(csvDir, 'refine_0kg_agile_r1.csv'));
fprintf('CSV 저장: %s\n', fullfile(csvDir, 'refine_0kg_agile_r1.csv'));
