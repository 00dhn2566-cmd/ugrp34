%% 0kg 앵커 좌표하강 2라운드 (18차): 자세 배율 x 고도 배율 격자 - 1차식 법칙의 g0 앵커 확보
%% 배경: r1 승자 sA=0.75/sZ=0.56 (추종 21.4cm, 오버 37cm, 유일 무발산)이 sZ 하단
%%       모서리에 걸림 - sZ를 아래로 확장(0.40/0.48)하고 sA는 0.75 주변(0.65/0.85) 미세화.
%% 판정: 격자 내부에서 최솟값이 잡히면 그 (sA*, sZ*)가 1차식 g0 앵커.
...
cfgs = { ...
    'sA0.65 sZ0.40', 0.65, 0.40; ...
    'sA0.65 sZ0.48', 0.65, 0.48; ...
    'sA0.65 sZ0.56', 0.65, 0.56; ...
    'sA0.75 sZ0.40', 0.75, 0.40; ...
    'sA0.75 sZ0.48', 0.75, 0.48; ...
    'sA0.75 sZ0.56', 0.75, 0.56; ...
    'sA0.85 sZ0.40', 0.85, 0.40; ...
    'sA0.85 sZ0.48', 0.85, 0.48; ...
...
nC = size(cfgs,1);
rows = nan(nC, 10);
fprintf('===== 0kg 앵커 2라운드: 자세 배율 x 고도 배율 (질량 0 고정, 이동 1m 젠틀무브 0.9s) =====\n');
...
writetable(Tb, fullfile(csvDir, 'refine_0kg_r2.csv'));
fprintf('CSV 저장: %s\n', fullfile(csvDir, 'refine_0kg_r2.csv'));
