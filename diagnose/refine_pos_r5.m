%% 위치 채널 미세조정 5라운드: 대각 원정 연장 (3라운드 kp 19까지 절벽 없음 - 더 바깥 탐색)
...
pairs = [30 12.0; 36 14.4; 43 17.2; 52 20.8];   % 첫 행 = 4R 끝점(기준점)
rows = nan(size(pairs,1), 8);
fprintf('===== 위치 5라운드: 대각 원정 연장 kd/kp=0.4 (ki=0.04 고정, 자세 새 게인 하) =====\n');
...
writetable(Tb, fullfile(csvDir, 'refine_pos_r5.csv'));
fprintf('CSV 저장: %s\n', fullfile(csvDir, 'refine_pos_r5.csv'));
