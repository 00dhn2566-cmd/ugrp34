%% 0 kg 재튜닝 3라운드: 결합 격자 (성능 지표 세션, 2026-08-18)
...
% --- 결합 격자 (r2 좌표하강 결론: 지터는 sA↓·r_att↓·limit_att↓ 에만 반응, 새그는 biasChassis 75(√질량 FF)로 해소) ---
G = {};
for sA = [0.40 0.45 0.50]
    for la = [50 100]
        for fmp = [0.005]
            for ra = [0.6 0.8]
                G(end+1,:) = {sA, la, fmp, ra}; %#ok<AGROW>
            end
        end
    end
end
...
rows = {};
fprintf('===== 0 kg r3b 결합 격자(sA 상향 경계): %d 구성 (FF √질량 75.5 / altCmdSat 30 / kp_pos 5 / filtPz 0.005 고정) =====\n', size(G,1));
fprintf('%5s %6s %5s %5s | %8s %8s %8s | %8s %8s %8s %8s | %7s\n', 'sA','limAtt','fMpos','r', '호버RMS','새그','드리프트','추종','오버','z피크','자세피크','점수');
for g = 1:size(G,1)
    cfg = BASE; cfg.sA = G{g,1}; cfg.limit_att = G{g,2}; cfg.filtM_pos = G{g,3};
    cfg.kp_pos = 5; cfg.kd_pos = 2.0; cfg.posErrSat = 1.2/5; cfg.posErrSatZ = 1.2/5;
    cfg.r_att = G{g,4}; cfg.biasChassis = 100.9*sqrt(1.2726/2.2726); cfg.altCmdSat = 30; cfg.filtPz = 0.005;   % √질량 FF 트림(0 kg = 75.5), Alt Cmd Sat 는 구운 값 유지
    [res, sc] = eval_cfg(cfg, mdl, mws, setTraj, tth, smH, ttm, smM, Th, Tm, sT, sQ);
    rows(end+1,:) = [G(g,:), num2cell(res), {sc}]; %#ok<AGROW>
    fprintf('%5.2f %6d %5.3f %5.1f | %8.3f %8.1f %8.2f | %8.2f %8.1f %8.1f %8.1f | %7.2f\n', G{g,:}, res, sc);
    Tb = cell2table(rows, 'VariableNames', {'sA','limit_att','filtM_pos','r_att','hover_att_rms_deg','sag_cm','drift_cm','track_rms_cm','overshoot_cm','z_peak_cm','att_peak_deg','score'});
    writetable(Tb, fullfile(csvDir, 'tune_0kg_r3b.csv'));
end
fprintf('완료: %.0fs\n', toc(t0all));
close_system(mdl, 0);
