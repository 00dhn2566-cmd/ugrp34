%% 0 kg 5라운드: 오차 의존 비선형 자세 게인 — 외란 이탈 '크기' 가두기 (성능 지표 세션, 2026-08-18, 사용자: "복귀는 포기, 이탈 범위 한정")
%% BASE = 현 0 kg 스케줄(sA 0.35 / kd:kp 0.6 / limit 100 / kp_pos 5 / filtPz 0.005 / FF √). Control Pitch/Roll PID 입력에
%% g(|e|) = 1 + (gmax-1)·sat((|e|-e0)/e1) 을 곱해(qc_nl_att_apply) 작은 오차(호버)는 그대로, 큰 오차(외란)만 권한 상승.
%% 격자: gmax x e0 (e1 = e0). 각 점: 호버 6 s + 1 m 이동 10 s + 외란 펄스 0.3 N·m 12 s (이탈 피크·수평 밀림·복귀).
%% 출력: diagnose/results/tune_0kg_r5_nl.csv   사용: matlab -batch "cd(fullfile(pwd,'diagnose')); tune_0kg_r5" > out.txt 2>&1
...
BASE = struct( ...
    ...
    'sZ', 0.56, 'kp_alt', 0.5, 'ki_alt', 0.1, 'kd_alt', 0.15, 'filtM_alt', 0.05, 'filtD_alt', 1000, 'filtPz', 0.005, ...
    'limit_alt', 10, 'altCmdSat', 30, 'biasChassis', 75.5, ...
...
% --- 비선형 자세 게인 격자 (gmax x e0) ---
qc_nl_att_apply(mdl);
GRID = [1.0 3; 1.6 3; 2.1 3; 2.9 3; 2.1 6; 2.9 6; 2.1 1.5];   % [gmax e0_deg], e1 = e0. 첫 행 = 항등(기준)
csvDir = fullfile(modelDir, 'diagnose', 'results');
if ~exist(csvDir, 'dir'); mkdir(csvDir); end
rows = {};
fprintf('===== 0 kg r5 비선형 자세 게인: %d 점 =====\n', size(GRID,1));
fprintf('%5s %5s | %8s %8s %8s | %8s %8s %8s %8s | %8s %8s %8s %8s\n', 'gmax','e0','호버RMS','새그','드리프트','추종','오버','z피크','자세피크','외란피크','회복s','밀림m','외란z');
for k = 1:size(GRID,1)
    cfg = BASE;
    assignin('base', 'nl_gmax', GRID(k,1)); assignin('base', 'nl_e0', deg2rad(GRID(k,2))); assignin('base', 'nl_e1', deg2rad(GRID(k,2)));
    set_param(plsB, 'Amplitude', '0');
    [res, ~] = eval_cfg(cfg, mdl, mws, setTraj, tth, smH, ttm, smM, Th, Tm, sT, sQ);
    set_param(plsB, 'Amplitude', '0.3');
    Td = 12; ttd = (0:round(Td/dt))' * dt; smD = repmat([0 0 1], numel(ttd), 1);
    setTraj(ttd, smD); set_param(mdl, 'StopTime', num2str(Td));
    dpk = NaN; trec = NaN; dz = NaN; dxy = NaN;
    try
        ...
        xg = gi(out.get('px')); yg = gi(out.get('py'));
        ...
        dxy = max(hypot(xg(post) - mean(xg(pre)), yg(post) - mean(yg(pre))));
    catch e
        fprintf('  (외란 실패: %s)\n', e.message);
    end
    set_param(plsB, 'Amplitude', '0');
    rows(end+1,:) = [{GRID(k,1), GRID(k,2)}, num2cell(res), {dpk, trec, dxy, dz}]; %#ok<AGROW>
    fprintf('%5.2f %5.1f | %8.3f %8.1f %8.2f | %8.2f %8.1f %8.1f %8.1f | %8.2f %8.2f %8.2f %8.1f\n', GRID(k,1), GRID(k,2), res, dpk, trec, dxy, dz);
    Tb = cell2table(rows, 'VariableNames', {'gmax','e0_deg','hover_att_rms_deg','sag_cm','drift_cm','track_rms_cm','overshoot_cm','z_peak_cm','att_peak_deg','dist_peak_deg','dist_recover_s','dist_xy_m','dist_z_cm'});
    writetable(Tb, fullfile(csvDir, 'tune_0kg_r5_nl.csv'));
end
fprintf('완료: %.0fs\n', toc(t0all));
close_system(mdl, 0);
