%% 타당성 축A 확장: 생 드론(0kg) x 정규화 ON/OFF (사용자 지적 17차: "과적합 금지, 생 드론 우선")
...
cfgs = { ...
    '기준 1.0kg   ON ', 1.0,  true;  ...
    '생드론 0kg   ON ', 1e-6, true;  ...
    '생드론 0kg   OFF', 1e-6, false; ...
};
...
nC = size(cfgs,1);
rows = nan(nC, 11);
fprintf('===== 축A 확장: 생 드론(0kg) x 정규화 A/B (이동 1m 젠틀무브 0.9s) =====\n');
...
fprintf('(판정: 0kg에서 OFF 우세면 "정규화는 탑재 시에만" 비대칭 규칙 전환 - 과적합 방지)\n');
...
writetable(Tb, fullfile(csvDir, 'validate_phys_ab0.csv'));
fprintf('CSV 저장: %s\n', fullfile(csvDir, 'validate_phys_ab0.csv'));
