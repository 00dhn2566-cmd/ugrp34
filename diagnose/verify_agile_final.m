%% agile 최종형 채택 관문 (18차): parameters.m 채택 경로 그대로 검증
%% 삼각 법칙 + z분리가 parameters.m에 반영된 상태에서, 게인 오버라이드 없이
%% ctrl_profile='agile' + qc_zsplit_apply만으로 (= 실사용 경로) 1kg 검증:
%% ① 호버 + 토크 펄스 0.3N·m x 0.3s @4s (z분리가 외란 응답 바꿨는지 확인)
%% ② 이동 1kg (튜닝 경로 실측 1.25cm 재현 = 채택 경로 무결 확인)
%% 합격: ① 생존+이탈<15도+회복<3s ② 추종 1.3cm급 + z이탈 한자릿수.
...
ctrl_profile = 'agile';            % 18차 최종형: 삼각 법칙 + z분리 (parameters.m이 계산)
quadcopter_package_parameters;
fprintf('parameters.m 산출: kp_pos=[%g %g %g] kd_pos=[%g %g %g] sat=%.3f satZ=%.3f\n', ...
    kp_position, kd_position, posErrSat, posErrSatZ);
mdl = 'quadcopter_package_delivery';
load_system(mdl);
qc_zsplit_apply(mdl);              % z분리 메모리 수술 (실사용 경로와 동일)
...
% 구성: {라벨, 질량kg, 이동?, 펄스암} — 게인은 parameters.m 산출 그대로 (오버라이드 금지!)
cfgs = { ...
...
nC = size(cfgs,1);
rows = nan(nC, 10);
fprintf('===== agile 최종형 관문: 채택 경로(parameters.m + qc_zsplit_apply) 1kg =====\n');
for c = 1:nC
    m_pkg = cfgs{c,2};
    ...
    if cfgs{c,3}
        ...
        rows(c,:) = [m_pkg, 1, 1, hovp, mv, ov, tailv, zpk, apk, 0];
        ...
        rows(c,:) = [m_pkg, 1, 1, surv, max(devR,devP), tRec, zpk, 0, 0, 1];
...
writetable(Tb, fullfile(csvDir, 'verify_agile_final.csv'));
fprintf('CSV 저장: %s\n', fullfile(csvDir, 'verify_agile_final.csv'));
