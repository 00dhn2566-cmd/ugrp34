...
for c = 1:nC
    ...
    % 자세 진동: 이동 중 피크(스텝~+5s) / 도착 잔류 RMS(+5s~+10s) / 말기 잔류 RMS(마지막 5s)
    rmsf = @(v) sqrt(mean((v-mean(v)).^2));
    segA = tu >= tStep & tu < tStep+5;
    segB = tu >= tStep+5 & tu < tStep+10;
    segC = tu >= T-5;
    attPk   = max(max(abs(pg(segA))), max(abs(rg(segA))));
    attMid  = rmsf(pg(segB));
    ...
    fprintf('%-10s | 자세: 이동피크 %5.2f도 | 도착직후 RMS(+5~10s) %5.3f도 | 말기 RMS(15~20s) %5.3f도\n', ...
        '', attPk, attMid, attTail);
    % 시계열 덤프 (판독용): 기준 vs 실측 + pitch
