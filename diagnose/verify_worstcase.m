...
C = {
 'A', '기준선 - 무지연 + 외란',                 0,   5, 1.00, AMP, [0 0 0]
 'B', '최악지연 - 스펙 안 깎음 + 외란',        60,  12, 1.00, AMP, [0 0 0]
 'C', '최악지연 - 표대로 0.75 + 외란',         60,  12, 0.75, AMP, [0 0 0]
 'D', '최악지연 - 더 깎음 0.55 + 외란',        60,  12, 0.55, AMP, [0 0 0]
 'E', '절벽지연 80ms - 안 깎음 + 외란',        80,  12, 1.00, AMP, [0 0 0]
 'F', '절벽지연 80ms - 표대로 0.37 + 외란',    80,  12, 0.37, AMP, [0 0 0]
 'G', '자세 관문 16ms - 표대로 0.75 + 외란',   60,  16, 0.75, AMP, [0 0 0]
 'H', '최악지연 - 안 깎음 - 외란 없음',         60,  12, 1.00, 0,   [0 0 0]
 'I', '최악지연 - 표대로 0.75 - 외란 없음',     60,  12, 0.75, 0,   [0 0 0]
 % --- 현실 대역 (사용자 지적 08-26: 60~80 ms 는 과한 가정) ---
 %     위치 지연만 변수, 자세는 5 ms 고정. A 가 0 ms 짝이다.
 'J', '현실대역 10ms + 외란',                  10,   5, 1.00, AMP, [0 0 0]
 'K', '현실대역 20ms + 외란',                  20,   5, 1.00, AMP, [0 0 0]
 % --- 힘 외란 / 복합 (사용자 요청 08-26) ---
 %   토크는 기체를 돌려서 간접적으로 밀지만, 바람은 직접 민다. 응답 모양이 다르다.
 %   크기 감각: 정면 0.2 m^2, Cd~1 이면 3 m/s 돌풍 = 1.1 N, 5 m/s = 3.1 N.
 'L', '20ms + 힘 y 2N (돌풍급)',              20,   5, 1.00, 0,   [0 2 0]
 'M', '20ms + 힘 y 5N (강한 돌풍)',           20,   5, 1.00, 0,   [0 5 0]
 'N', '20ms + 복합(토크x + 힘 y,z)',          20,   5, 1.00, AMP, [0 2 -2]
...
for i = 1:size(C,1)
    ...
    e.lab = lab; e.desc = desc; e.tauP = tauP; e.tauA = tauA; e.s = s; e.amp = amp;
    e.fvec = fvec;
