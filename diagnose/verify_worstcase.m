...
for i = 1:size(C,1)
    ...
    % ★ 08-29 실측: **이 훅은 안 듣는다.** WC_WIND=5 로 돌린 행과 무풍 행이
    %   소수점까지 동일했다 (추종 8.19 / 외란y 0.38 / est 0.5888 / act 0.1635).
    %   base 의 wind_speed 를 구운 모델이 안 읽는 것으로 보인다 — 바람 블록이
    %   상수로 구워졌거나 다른 경로를 쓴다. 고치기 전까지 '바람에서 깎기가
    %   값어치를 하나' 는 **미측정**이다 (무효가 아니라 못 잰 것).
    %   할 일: 모델에서 wind 관련 블록을 찾아 파라미터를 직접 set_param 할 것.
    if ~isempty(getenv('WC_WIND'))
