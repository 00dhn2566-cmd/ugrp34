...
> **2026-08-29 후속**: 이 문서의 A1~A7을 잇는 최신 안건은 [meeting_agenda_2026-08-29.md](meeting_agenda_2026-08-29.md) — 2학기 마일스톤·시뮬레이터 역할 재설정이 추가됐고, 잠정 확정 6건은 일괄 추인 항목으로 묶였다. 근거 문서 링크는 이 문서가 여전히 기준.
...
> **2026-08-18 갱신 — 잠정 확정 +2건 (루트 spec)**: ⑤ **창문의 시각 정의** = 색 테두리 프레임 + 뚫린 개구부, corner = 개구부 모서리 (§2.5 — 윤호 PyBullet 실측: 채움은 색 판정·삼각측량 붕괴) → A2는 **추인만** ⑥ **카메라-IMU 외부 파라미터·pose 기준 프레임** = IMU/body 프레임, body x전방/y좌/z상, 전방 카메라 표준 T_IC, 태민 상수 단일 진실 (§6.1 — EuRoC 값이 "위를 보는 카메라"로 읽히는 문제) → **A2-c 신설, 추인만**. 신규 열린 안건: **A2-d 검출 클래스 설계** (단일클래스+HSV vs 3클래스 — spec §7).
>
